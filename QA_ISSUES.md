...
**Fix:** New `claimAppliedFollowupSend` in `src/lib/sms-member-registry.js`: a durable Redis SET NX claim with a 24h TTL, keyed `appt:<appointmentId>:<offerKind>` (falling back to `phone:<last10>:<offerKind>` when no appointment id survived), so the same appointment can still receive one duration confirmation AND one add-on confirmation while a same-kind repeat stays deduped. The webhook takes the claim AFTER the send-time STOP gate (a suppressed pass never consumes it) and BEFORE the Twilio send; anything but a fresh claim withholds the send (fail closed, at-most-once). Gauntlet follow-up (Codex P1): because the claim is itself a network await between the STOP check and the send, the route re-checks the strict STOP gate AFTER the claim and sends only on a second affirmative `off`, so a STOP landing during the claim still wins. Accepted residual window: a STOP processed on another instance in the milliseconds between the recheck and the Twilio API call can still lose; that gap cannot be closed client-side, and Twilio's carrier-level opt-out on standard keywords is the backstop. Accepted trade-off (owner-specified at-most-once): a claim consumed by a send that then fails or crashes stays consumed for the TTL; the member still holds the manual-confirm ack. Tests: `__tests__/sms-followup-claim.test.js` (NX semantics, TTL, fail-closed paths) plus route-level double-delivery, claim-not-burned-on-STOP, claim-refused, and STOP-during-claim tests in `__tests__/twilio-webhook-route.test.js`.
...
**Fix:** A single shared `maskPhoneDigits` (exported from `src/lib/sms-member-registry.js`) masks anything phone-shaped (8+ digits) down to `***` plus the last 4 before logging. Applied to: all four send-path error catches in the webhook route (stop-set check threw, send claim threw, stop-set recheck threw, Twilio send failed), the STOP/START handler logs and catches, the Klaviyo unsubscribe warn, the deferWork backstop catch, the top-level webhook catch, the phone-index logs, and every catch inside the registry itself (a Redis client error can echo the full command carrying the number; the registry's own stop-set failure logs were the actual leak surface for send-path Redis errors, since `checkStopSetStrict` swallows its errors before the route's masked catch can fire). Registry happy-path logs (`Added`, `Removed N entries`) now log last-4 only. Tests force each error with a full E.164 embedded and assert no full number reaches the logs. Known remaining PII surface, out of scope here: the Google Sheets logs store the full outbound phone by design (see `docs/MONITORING_SUMMARY_sms-path1.md` item 3), and incident rows carry the full phone on purpose so the team can act on them.

---

### outbound-sms #18
**Status:** FIXED IN CODE 2026-07-22 with a DOCUMENTED RESIDUAL (branch `fix/carveout-followup-hardening`, awaiting Matt's merge)
**Severity:** customer-harm (a polite refusal could enqueue a real paid apply) plus compliance (phrase-level opt-outs went unhonored)
**Discovered:** 2026-07-22 (red team, then codex adversarial rounds 5 through 14)

**Symptom:** The YES/NO keyword lane classified "No thanks, please stop texting me" as a YES (affirmative won ties and `please`/courtesy words were YES keywords), enqueuing a real Boulevard apply for a refusing member. Phrase-level opt-outs ("please stop texting me", "Don't text me", "STOP,") bypassed the bare-keyword STOP handler entirely, leaving no suppression record anywhere.

**Fix:** Deterministic consent classifier in the webhook route: negative wins ties; `please` removed from YES keywords; smart apostrophes normalized; all trailing punctuation stripped before the bare-keyword test; unambiguous sender-directed SMS opt-out phrases get the FULL STOP treatment, evaluated per clause with verb-attached negation (including won't/can't/not-ready-to and adverbs), third-party person exclusions, and non-SMS-target exclusions (email, newsletter, membership, account, appointment, waitlist); refusal keywords are tested with negated verb chains stripped so "Yes, don't skip it" stays a YES. Roughly 30 phrasings pinned in `__tests__/twilio-webhook-route.test.js` (`intent classification safety`).

**DOCUMENTED RESIDUAL (accepted, not closable by regex):** English phrasing space is unbounded, and ten codex review rounds kept generating novel variants of the same classes (new negation forms, new third-party shapes, new channel targets). The remaining failure modes are bounded in severity: over-suppression is self-disclosing (the member receives the unsubscribe confirmation) and reversible (START), under-suppression falls through to the chat bot where a human-quality model handles it, and the bare-keyword carrier-standard STOP path is unaffected. The structural fix, if Matt wants one, is a cheap LLM intent-classification call for consent-ambiguous inbound messages (the repo already runs Claude for chat); that is a separate scoped project, not a regex patch.

---

### outbound-sms #19
**Status:** FIXED IN CODE 2026-07-22 (branch `fix/carveout-followup-hardening`, awaiting Matt's merge)
**Severity:** customer-harm (duplicate booking edits and duplicate price changes)
**Discovered:** 2026-07-22 (codex adversarial, gauntlet on this branch; owner directed the fix)

**Symptom:** The durable dedup shipped as #16 guarded only the follow-up SMS, and its claim was acquired AFTER `reverifyAndApplyUpgradeForProfile` had already mutated the booking. Two racing YES deliveries (Twilio redelivery, or a double YES landing on separate serverless instances before either saw the other's pending-offer clear) could both run the Boulevard mutation on the same appointment: duplicate booking edits behind a single confirmation.

**Fix:** New `claimApplyMutation` in `src/lib/sms-member-registry.js`: a durable Redis SET NX claim taken BEFORE the Boulevard write, keyed via the shared `buildApplyClaimKey` as `appt:<appointmentId>:<offerKind>:<target>` (target = duration minutes or add-on slug, segments sanitized; `phone:<last10>` scope fallback), TTL 600s (2x the webhook's 300s maxDuration, the ceiling on any in-flight apply). Applied at BOTH apply sites: the webhook's deferred worker AND the chat route's inline apply (`src/app/api/chat/message/route.js`), which the gauntlet proved reachable from the same Twilio number ("Please!" falls through to chat, whose own YES keywords still include "please") and racing the deferred apply. Tri-state with crash-window state tracking: `claimed` proceeds to the write and flips the claim value from `pending` to `settled` once the apply produces an outcome; `held` inspects the claim state, skipping silently only when the owner is settled or pending inside the 330s in-flight ceiling, and otherwise (abandoned or unreadable) queues an `apply_claim_abandoned` incident WITHOUT re-applying (Boulevard state unknown); `unavailable` (Redis down) also does NOT write and queues an `apply_claim_unavailable` incident so a human applies the change and the "our team will confirm" promise stays honest. An apply that THROWS after claiming queues an `apply_threw_after_claim` incident and then settles; on EVERY failure outcome the incident is durably queued BEFORE the claim settles (a worker dying after settle but before the incident would hide the failure behind a settled claim), and the chat route now queues an incident on non-throw apply failures too, since its unavailable copy promises team follow-up. The webhook's early-return claim paths still write the ack audit row. A genuine second, DIFFERENT change to the same appointment (add-on after a duration upgrade) uses a different key and still applies. Tests: race repro (two concurrent deliveries, slow apply, exactly one mutation and one confirmation), claim-before-write ordering on both the pending-offer and re-derive paths, held-skip, abandoned and unavailable incident paths, throw-after-claim, settle ordering, phone-fallback key, chat-route claim wiring, plus registry-level NX/TTL/state tests in `__tests__/sms-followup-claim.test.js`.
