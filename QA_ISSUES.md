...
**Last updated:** July 16, 2026 (cancel-bot #32 and #33 shipped on branch `fix/prompt-booking-support`: booking support capture flow replacing the canned reply, credit-transfer policy removed, custom-pause lock, submit-not-execute action language, cancellation timelines removed, member pricing in the services overview. Awaiting Matt's review, not merged.)
...
**System:** cancel-bot
**Severity:** trust-erosion (over-promised action), compliance-risk (unverifiable timelines)
**Status:** FIXED IN CODE 2026-07-16 by PR `fix/prompt-booking-support`, awaiting Matt's review. Bump to VERIFIED FIXED after merge + production deploy.

**1. The bot claimed to do things it cannot do.** The bot has no write access to Boulevard. The memberships team executes every cancellation, pause, downgrade, and bi-monthly switch by hand after the session ends. The prompt nonetheless had the bot say "I'm processing your cancellation now", "I'm setting up the 2-month pause now", and "Your cancellation is processed" across roughly 20 sites, including its GOOD examples. At the moment the bot said it, the team had not seen the request. New HARD RULE - THE BOT SUBMITS REQUESTS, IT DOES NOT EXECUTE THEM, and every site rewritten to "I'm submitting your cancellation request to our memberships team now." Wording only: the cancellation is still honored on the spot, and the rule says explicitly that the submit framing is never a reason to re-confirm or reopen retention, because added friction on a cancellation is an FTC Negative Option problem. "I can't do that in chat" stays banned.

**2. Unverifiable timelines removed.** "Processing time is 30 days", "processing takes 30 days", "30-day processing", and the "30 days written notice" day count are gone, along with any predicted end date or final charge. New HARD RULE - NO BILLING DATE PREDICTIONS. The bot references the written-notice policy generally and routes specifics to the team. NOTE this REVERSES a decision an earlier PR locked in: the 30-day notice was preserved as "the legal notice period, a real defined policy", was explicitly allowed by HARD RULE - NO HUMAN-TEAM SLA PROMISES, and had tests pinning it. Those allowances are withdrawn per Matt's call. The 90-day credit validity and the perk claim windows are deliberately kept: those are defined policy, not cancellation timing.

**3. Member pricing in the services overview.** A prospect asking "what services do you offer" should see that membership lowers the per-visit cost. Finding worth recording: the canonical menu was ALREADY correct (tier headers have always carried both prices, and the tokens resolve in General Mode). The gap was behavioral: nothing told the bot to quote both, and the worked examples in STYLE RULES and the first-time recommendation modeled walk-in only. Fixed with a MEMBER PRICING IS ALWAYS SHOWN rule, a single "Members save on every visit" line (explicitly not a pitch), and both prices in the examples via the centralized tokens.

**Gauntlet findings fixed in the same PR, worth knowing about:** the new billing rule initially claimed the bot "cannot see the member's billing schedule", which is false (`boulevard.js:4291` injects Next Charge Date), and told the bot to say so to members; it also promised the confirmation email "will confirm the exact dates", an outcome promised on the team's behalf that the bot cannot know. Both corrected. Residual "I can set up here" survived in the custom-pause Example GOOD, and "Done." was still an approved acknowledgment. Most importantly the regression guard itself was too permissive to guard: measured against the pre-change prompt it caught 9 of 22 offenders, so the two most-copied sites could have regressed with CI green. Rebuilt around structural exemptions and now catches 13 of 13.

**Open question for Matt (not blocking, but it matters):** the session-end email (`processConversationEnd` -> `sendSummaryEmail`) goes to memberships@ and hello@. The member is never a recipient; `member-draft.js` builds a draft that Fernanda sends by hand. So "you'll receive a confirmation email" is true only if the team sends that draft every time. That promise is now doing more work, since the bot no longer states timing itself and points at the team instead. Worth confirming the manual step is reliable, or the promise is the next Issue 6.
